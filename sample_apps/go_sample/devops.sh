...
go run query-sample.go -query "SELECT region, az, hostname, BIN(time, 15s) AS binned_timestamp,
...
go run query-sample.go -query "WITH avg_fleet_utilization AS (
...
go run query-sample.go -query "SELECT BIN(time, 30s) AS binned_timestamp, ROUND(AVG(measure_value::double), 2) AS avg_cpu_utilization,
hostname FROM $1.$2
WHERE measure_name = 'cpu_utilization'
    AND hostname = '$3'
    AND time > ago(2h)
...
go run query-sample.go -query "WITH binned_timeseries AS (
...
go run query-sample.go -query "WITH per_host_min_max_timestamp AS (
...
go run query-sample.go -query "WITH time_series_view AS (
