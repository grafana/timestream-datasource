...
test('should successfully create an alert rule', async ({
  ...
  // Seems there is a race condition where the onBlur from the SQLEditor triggers a POST to '/api/v1/eval'
  ...
  await alertRuleEditPage.ctx.page.mouse.click(0, 0); // forces a click outside the SqlEditor forcing the onBlur handler
  await alertRuleEditPage.ctx.page.waitForRequest((req) => req.url().includes(selectors.apis.Alerting.eval)); // wait for the onBlur handlers call to '/api/v1/eval'
